  ...
  for (let i = a; i <= b; i++) {
    ...
    if (i % 100 === 0) { // Adjust the interval as needed for better performance
      ...
      if (energyConsumptionmWh > energyLimit) {
        ...
        httpRequest('PATCH', `http://localhost:8086/api/task/executed/${task_id}`, { status: 3, hash: scriptHash }, () => {
          process.exit(1);
        });
...
httpRequest('GET', `http://localhost:8086/api/task/${task_id}`, null, (err, res, body) => {
  ...
  if (response.hash !== scriptHash) {
    ...
    httpRequest('PATCH', `http://localhost:8086/api/task/executed/${task_id}`, { status: 4, hash: scriptHash }, () => {
      process.exit(1);
    });
  } else {
    httpRequest('PATCH', `http://localhost:8086/api/task/executed/${task_id}`, { status: 1, hash: scriptHash }, (err) => {
      if (err) {
        console.error('Error making API call:', err);
        process.exit(1);
      }
      ...
      httpRequest('PATCH', `http://localhost:8086/api/task/executed/${task_id}`, { status: parseInt(finalStatus), hash: scriptHash, executionTime: parseFloat(executionTimeHours.toFixed(2)), energyConsumed: parseFloat(energyConsumptionmWh.toFixed(3)) }, () => {
