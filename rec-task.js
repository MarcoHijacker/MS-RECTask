...
function getSystemPowerConsumptionRate() {
    ...
    const totalPowerConsumptionW =
        basePowerConsumptionW +
        numCores * powerPerCoreW +
        totalMemoryGB * powerPerGBRamW;
...
function findPrimesBetween(a = 0, b = 1000000000, energyLimit) {
    ...
    for (let i = a; i <= b; i++) {
        ...
        if (i % 100 === 0) {
            // Adjust the interval as needed for better performance
            ...
            const energyConsumptionmWh =
                totalPowerConsumptionmWh * executionTimeHours;
            ...
            if (energyConsumptionmWh > energyLimit) {
                ...
                console.log(
                    ...
                    energyConsumptionmWh.toFixed(3)
                );
                console.log("Energy limit quota (mWh):", energyLimit);
                console.log(
                    "Delta between limit quota and consumed energy (mWh):",
                    (energyLimit - energyConsumptionmWh).toFixed(3)
                );
                console.log(
                    "Program terminated due to exceeded mWh limit quota."
                );

                httpRequest(
                    "PATCH",
                    `http://localhost:8086/api/task/execution/${task_id}`,
                    { status: 3, hash: scriptHash },
                    () => {
                        process.exit(1);
                    }
                );
...
httpRequest(
    "GET",
    `http://localhost:8086/api/task/${task_id}`,
    null,
    (err, res, body) => {
        if (err) {
        ...
        if (response.hash !== scriptHash) {
            ...
            httpRequest(
                ...
                (err) => {
                    ...
                    if (isNaN(a) || isNaN(b)) {
                        console.error(
                            "Please provide valid numbers for a and b."
                        );
                        process.exit(1);
                    }
                    ...
                    console.log(
                        `Estimated Power Consumption Rate (mWh per hour): ${totalPowerConsumptionmWh}`
                    );
