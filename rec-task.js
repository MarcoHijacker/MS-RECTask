...
httpRequest('GET', `http://localhost:8086/api/task/${task_id}`, null, (err, res, body) => {
  ...
  if (response.hash !== scriptHash) {
    ...
    httpRequest('PATCH', `http://localhost:8086/api/task/executed/${task_id}`, { status: 1, hash: scriptHash }, (err) => {
      ...
      httpRequest('PATCH', `http://localhost:8086/api/task/executed/${task_id}`, { status: finalStatus, hash: scriptHash, executionTime: parseFloat(executionTimeHours), energyConsumed: parseFloat(energyConsumptionmWh.toFixed(3)) }, () => {
