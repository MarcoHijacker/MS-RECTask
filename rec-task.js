...
httpRequest('GET', `http://localhost:8086/api/task/${task_id}`, null, (err, res, body) => {
  ...
  if (response.hash !== scriptHash) {
    ...
    httpRequest('PATCH', `http://localhost:8086/api/task/execution/${task_id}`, { status: 1, hash: scriptHash }, (err) => {
      ...
      httpRequest('PATCH', `http://localhost:8086/api/task/execution/${task_id}`, { status: parseInt(finalStatus), hash: scriptHash, executionTime: parseFloat(executionTimeHours.toFixed(2)), energyConsumed: parseFloat(energyConsumptionmWh.toFixed(3)) }, () => {
